...
function register() {
    ...
    if(password!=finalPassword){
        $('#final_password').attr("data-content","确认密码与密码不一致");
        $('#final_password').popover('show');
        return;
    }
    $.ajax({
		...
		success:function (data) {
			if(data['success']==true){
                ...
                if (data['name_null']) {
                    ...
                    $('#regist_name').attr('data-content', '用户名已存在');
                    $('#regist_name').popover('show');
                }
            }
        }
	})
		...
		success:function (data) {
			if(data['success']==true){
				...
				if(data['authority']==false){
					alert('没有权限');
					logout();
				}else if(data['last_password_error']==false){
					$('#last_password').attr("data-content","原始密码错误");
					$('#last_password').popover('show');
				}
			}
        }
